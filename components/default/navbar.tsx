    ...
    >
      <div className="container mx-auto px-4 py-4 flex items-center justify-between">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
          <Link href="/" className={`text-xl tracking-tight hover:opacity-80 transition-colors ${getLogoStyles()}`}>
            Portfolio
          </Link>
        </motion.div>
          ...
          {navItems.map((item) => (
            <button
              ...
              className={`relative px-4 py-2 text-sm rounded-full transition-colors ${
                activeSection === item.id ? getActiveNavStyles() : `text-foreground/70 ${getNavHoverStyles()}`
              }`}
            >
