...
// Default site data for client-side rendering
export function getDefaultSiteData(): SiteConfig {
    return {
        siteName: 'Portfolio',
...
// Client-side function to get site data
export async function getSiteData(): Promise<SiteConfig> {
    try {
        // In a client component, fetch from API
...
// Client-side function to update site data
export async function updateSiteData(newData: SiteConfig) {
    try {
        const response = await fetch(`${baseUrl}/api/site`, {
            method: 'PUT',
            headers: {
        ...
        return { success: false, error: 'Failed to update site data' }
    }
}

// Client-side function to update a specific section
export async function updateSiteSection<T>(section: keyof SiteConfig, data: T) {
    try {
        const response = await fetch(`${baseUrl}/api/site/${section}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        return { success: false, error: `Failed to update ${section} data` }
    }
}
